...
**El tono tenue es para texto grande.** axe midió lo que ningún repaso de capturas vio: `--tinta-tenue` (#78839a, de la entrega) da 3,6:1 sobre el fondo claro — de sobra para titulares (3:1), corto para texto pequeño (4,5:1). La paleta no se toca, porque el test de paleta la ata a la cartelería; lo que manda es el **uso**: las versalitas, ayudas, metas y cuerpos pequeños visten `--tinta-suave`, y `--tinta-tenue` queda para texto grande y adornos. Lo vigila la auditoría axe del CI, bloqueante sobre la landing y sobre **todas** las pantallas del panel —la lista la da el directorio `src/app/panel`, así que una pantalla entra en la auditoría por existir; antes salía de los módulos de primer nivel y se dejaba fuera las de dentro, que son las densas— — y ya ha cazado un segundo caso de la misma familia en un módulo recién escrito: `--aviso-tinta` era el ámbar de la entrega (3,25:1) y las alergias del reparto se avisan con él en cuerpo pequeño. El color del aviso (`--aviso`) se queda; la **tinta** del aviso pasa al bronce 600 de la misma entrega, que llega a 5,4:1. El verde y el rojo no lo necesitan: medidos, dan 5,3:1 y 4,5:1.
...
**Estas seis tablas las escribe el panel, en el módulo «Contenido», y ya no queda nada de la landing que sólo se toque por SQL.** `hitos_programa` —con su conmutador de víspera y día de la boda—, `rutas_llegada`, `consejos_vestimenta` y `preguntas_frecuentes` desde BODA-129; `hitos_historia` y `alojamientos`, que llevan `medio_id`, desde BODA-130. De ahí salió #163: secciones encendidas que no aparecían porque no había por dónde llenarlas.

**Las dos con foto no suben nada: eligen.** El selector ofrece las imágenes ya subidas, publicadas y de la sección que toca, porque la landing pide `m.publicado` **en la condición del `join`** —un hito con foto en borrador sale sin foto, no desaparece— y ofrecer un borrador enseñaría en el panel una imagen que la web no pinta. Subir, el texto alternativo obligatorio y el borrado del fichero siguen viviendo en «Fotos y vídeos», que es su sitio. `medio_id` es `on delete set null`: borrar una foto deja el hotel en pie, sin imagen.
...
**El paisaje es una escena de scroll, no una franja.** La entrega lo pone bajo la portada como una sección de **tres pantallas de alto** con un panel pegado dentro: al bajar, un recuadro centrado del 40 % de ancho se abre hasta ocupar la pantalla entera, la foto se aleja, el velo se oscurece y la frase entra por el centro a mitad de recorrido (BODA-117). **La mueve el scroll, no JavaScript**: la sección declara una línea de tiempo con nombre (`view-timeline-name`) y sus hijos la consumen; el tramo `contain 0 → 100 %` de un elemento más alto que la pantalla es exactamente el recorrido que la entrega calcula a mano. **El marco se abre recortando y no creciendo** —`clip-path` en vez de `width`/`height`— porque animar una caja obliga a recalcular la maquetación en cada fotograma y mete saltos en el CLS; la sombra va en `drop-shadow` por lo mismo, que una `box-shadow` se recortaría con el marco. Donde el navegador no sepa de líneas de tiempo de scroll, y **con `prefers-reduced-motion`, no hay escena**: la sección mide lo que medía y la frase está a la vista desde el principio — nunca se esconde nada esperando un scroll que quizá no ocurra. Esto sustituye la decisión anterior («el paisaje no llega a pantalla completa, se tiene que ver que hay algo debajo»): si los novios prefieren la franja estática, se revierte esa PR sola.

**La frase son tres líneas, y manda el titular.** La entrega las escribe con tres tipografías distintas —versalita en marino claro, titular en serif blanco, cierre en cursiva bronce—, así que son tres campos de `configuracion_boda` (`paisaje_intro`, `paisaje_titulo`, `paisaje_cierre`) y no un texto partido por palabras: de una sola frase no hay forma de saber dónde corta cada línea. **Son datos de la boda** —nombran tres ciudades concretas— y se editan desde ajustes, no en `copy.es.json`, que es el sitio de lo que no cambia de una boda a otra. Sin titular no hay sección: una vista aérea muda es un fondo bonito que no dice nada. La versalita y el cierre son opcionales una a una, y la foto tampoco manda: sin ella la sección se sostiene sobre el plano de marca que la propia entrega pinta detrás del marco, porque la frase es el mensaje y la foto es cómo se presenta. **El titular es el `h2`**: es lo único que la sección dice, y lo que le da nombre para quien la recorre saltando de titular en titular. El cierre es la segunda —y última— aparición de Italianno en la Landing, que la propia entrega usa dos veces pese a su regla de «una vez por pieza»; pasa por `<Conector>` como la «y» de la portada, que es el único componente que puede nombrar esa letra.
...
**El movimiento es el de la Landing aplicada, sin JavaScript** (BODA-118 a BODA-120). La portada entra escalonada con siete retardos con token (`--retardo-portada-1…7`, de 0,2 a 1,6 s) y la curva de la entrega (`--ease-salida`); la foto de portada aparece en fundido, hace el zoom lento de 2,4 s y baja con el scroll hasta 60 px sangrando un 6 %; la pista «bajad» cae y se apaga. La barra de lectura de 2 px y la cabecera que pasa de transparente a velada en 90 px van ligadas al scroll con `animation-timeline: scroll(root)`: donde el navegador no lo soporte, la barra no se pinta y la cabecera queda velada. Las tarjetas se levantan al pasar (`elevar-al-pasar`, con la sombra de la entrega), las fichas de la playlist entran con el «pop» de muelle (`--ease-muelle`), las fotos de la tira se descubren con barrido y viajan con paralaje, el monograma del pie hace «pop» y el cielo deriva en 44 s exactos con tokens de periodo. Con `prefers-reduced-motion` se anulan también los retardos y los `hover` que mueven; la barra de lectura se queda porque mide, no decora. Los rangos de los reveals (`entry 0% cover 30%`) se conservan: están razonados en `motion.css` (BODA-126).

**Los cuatro componentes del catálogo existen de verdad** (BODA-123). El sistema de marca entrega botones, campos, etiquetas y avisos, y tarjeta; hasta este ticket el repo tenía los dos primeros y pintaba los otros dos a mano en cada pantalla, con veinte copias que ya no eran iguales entre sí. Ahora son `EtiquetaEstado`, `Aviso` y `Tarjeta` en `src/components/ui/`, y `/cocina` los enseña con los rótulos de la entrega y dentro de un bloque inverso, que es donde se ve si alguno lleva un color escrito en la clase.

`EtiquetaEstado` es la única píldora del proyecto: un test unitario falla si una pantalla vuelve a escribir `rounded-etiqueta` junto a un fondo, con tres excepciones enumeradas y razonadas (una barra de muestra, un chip con casilla y el contorno de una mesa del plano, que llevan ese radio por la forma y no por ser etiquetas). Tiene más variantes y más tamaños que los cuatro de la entrega, y cada uno se gana el sitio: la entrega diseña la web pública, donde nada puede ir mal, y el panel sí tiene avisos y errores. Las dos «marcadas» escriben en tinta y llevan el color al borde porque `--error-tinta` sobre su fondo da 4,1:1 y `--aviso-tinta` 2,9, los dos por debajo de AA para el texto más pequeño de la pantalla.

**Dos estados que no coincidían con la entrega.** El campo llevaba `focus:outline-none`, que anulaba el aro de bronce del `:focus-visible` global y dejaba como único aviso de foco un borde que cambia un punto de color; ahora conviven el aro de fuera y el anillo marino de 3 px de la entrega (`--anillo-campo`). Y el botón desactivado se pintaba a media opacidad, que vuelve el relleno traslúcido y da un gris distinto en cada fondo; ahora tiene color propio (`--accion-desactivada`, `--tinta-desactivada`), reasignado en el bloque inverso y en el pie, donde la acción es clara y lo apagado tiene que oscurecerse en vez de aclararse.

**Una nota sobre el contraste de lo desactivado.** El par que escribe el estudio da 2,48:1, por debajo del 3:1 que pedía el ticket. Son los colores de la entrega y la 1.4.3 de las WCAG deja fuera el texto de un control inactivo, así que se han montado tal cual; lo que el test sí exige es que un botón apagado no se parezca a uno encendido en ninguno de los tres fondos, que es el defecto real que tenía `opacity-50`.
...
| **Contenido**      | Qué secciones se enseñan y en qué orden, qué le falta a cada una para verse y dónde se escribe; y la escritura de las seis listas: programa y víspera, cómo llegar, dress code, preguntas frecuentes, nuestra historia y dónde dormir —estas dos eligiendo foto entre las ya subidas— |
