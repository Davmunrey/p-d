...
**El bucket de Storage también es una migración.** Se descubrió montando el gestor de medios: `storage.buckets` estaba **vacío en el proyecto remoto** — no es que el bucket viviera configurado a mano y sin versionar, es que no existía, y por eso la portada nunca había podido tener foto. Ahora lo crea `20260811090000_bucket_medios.sql` con sus políticas, y una base recién levantada reproduce el almacenamiento igual que reproduce las tablas. Para poder probarlo de verdad, `probar-bbdd.sh` y `preparar-bbdd.sh` recrean el esquema `storage` igual que ya recreaban `auth`: no se simula el servicio, se recrean las TABLAS sobre las que actúan las políticas.

**El bucket es público para leer, y eso es una decisión, no un descuido.** Una foto se pinta con `<img src>` y un navegador no manda la clave de la API, así que un bucket privado obligaría a firmar cada URL en cada pintado. El precio, dicho claro: quien conozca la ruta exacta de un fichero puede verlo aunque su fila esté sin publicar — publicar controla lo que sale en la web, no esconde el objeto. Se mitiga con rutas que llevan un identificador aleatorio. Escribir no se cede: sube quien puede editar, dentro de una carpeta cuyo nombre sea una sección, y **el prefijo se comprueba contra el enumerado `seccion_landing`**, no contra una lista escrita en la política — así, añadir una sección no exige una migración que la persiga. Cinco comprobaciones bloqueantes lo sujetan, incluida que `anon` no puede subir nada ni conociendo la ruta.
